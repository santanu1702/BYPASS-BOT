...
from pyrogram.enums import ParseMode  # Added the strict enum import
...
def user_guard(func):
    ...
    async def wrapper(client: Client, message: Message):
        ...
        if get_setting("maintenance") and not admin:
            await message.reply("🔧 *Bot is under maintenance.* Please try again later.", parse_mode=ParseMode.MARKDOWN)
            return
        ...
        if not admin and not vip:
            ...
            if cd > 0:
                m = await message.reply(f"⏳ *Cooldown!* Please wait *{cd}s*.", parse_mode=ParseMode.MARKDOWN)
...
async def cmd_maintenance(client: Client, message: Message):
    ...
    await message.reply(f"🔧 Maintenance mode: {'*ON*' if val else '*OFF*'}", parse_mode=ParseMode.MARKDOWN)
...
async def cmd_setcooldown(client: Client, message: Message):
    ...
    try:
        ...
        await message.reply(f"✅ Cooldown set to *{secs}s*.", parse_mode=ParseMode.MARKDOWN)
    except ValueError:
...
async def cmd_removecooldown(client: Client, message: Message):
    ...
    await message.reply("✅ Cooldown *removed*.", parse_mode=ParseMode.MARKDOWN)
...
async def cmd_setlimit(client: Client, message: Message):
    ...
    try:
        ...
        await message.reply(f"✅ Daily limit set to *{n}*.", parse_mode=ParseMode.MARKDOWN)
    except ValueError:
        await message.reply("❌ Invalid number.")
...
async def cmd_removelimit(client: Client, message: Message):
    ...
    await message.reply("✅ Daily limit *removed*.", parse_mode=ParseMode.MARKDOWN)
...
async def cmd_addadmin(client: Client, message: Message):
    ...
    await message.reply(f"✅ `{uid}` added as admin ({dur}).", parse_mode=ParseMode.MARKDOWN)
    try:
        await client.send_message(uid, f"🛡️ You have been *added as Admin* ({dur}).", parse_mode=ParseMode.MARKDOWN)
    except Exception:
        pass
...
async def cmd_removeadmin(client: Client, message: Message):
    ...
    if len(s["admins"]) < before:
        await message.reply(f"✅ `{uid}` removed from admins.", parse_mode=ParseMode.MARKDOWN)
        try:
            await client.send_message(uid, "🚫 Your *Admin access* has been removed.", parse_mode=ParseMode.MARKDOWN)
        except Exception:
            pass
    else:
...
async def cmd_addvip(client: Client, message: Message):
    ...
    dur = f"{days} days" if days else "permanent"
    await message.reply(f"⭐ `{uid}` is now VIP ({dur}).", parse_mode=ParseMode.MARKDOWN)
    try:
...
async def cmd_removevip(client: Client, message: Message):
    ...
    await message.reply(f"✅ VIP removed from `{uid}`.", parse_mode=ParseMode.MARKDOWN)
    try:
        await client.send_message(uid, "❌ Your *VIP access* has been removed.", parse_mode=ParseMode.MARKDOWN)
    except Exception:
        pass
...
async def cmd_ban(client: Client, message: Message):
    ...
    await message.reply(f"🚫 `{uid}` has been *banned*.", parse_mode=ParseMode.MARKDOWN)
...
async def cmd_unban(client: Client, message: Message):
    ...
    await message.reply(f"✅ `{uid}` has been *unbanned*.", parse_mode=ParseMode.MARKDOWN)
...
async def cmd_addbypass(client: Client, message: Message):
    ...
    save_settings(s)
    await message.reply(f"✅ `{domain}` added. The bot will now attempt to bypass links from this domain.", parse_mode=ParseMode.MARKDOWN)
...
async def cmd_removebypass(client: Client, message: Message):
    ...
    if domain in s["custom_bypasses"]:
        ...
        save_settings(s)
        await message.reply(f"✅ `{domain}` removed.", parse_mode=ParseMode.MARKDOWN)
    else:
